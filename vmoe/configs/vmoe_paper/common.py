...
DESCRIPTIONS_REGEX = re.compile(
    ...
    r'K=(?P<k>[0-9]+), '
    r'(?P<where>Every|Last|Custom) (?P<where_num>[0-9]+), '
    r'(?P<epochs>[0-9]+) Epochs, '
    r'(?P<momentum>Momentum|Base)$'
    )
...
def get_vmoe_config(description: str, image_size: int,
  ...
  elif moe_where == 'Custom':
    config.encoder.moe.layers = (1,2,3,5,6,7)
  else:
    raise ValueError(
