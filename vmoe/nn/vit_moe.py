...
from vmoe.vit_jax import models_vit
import vmoe.moe
...
class EncoderBlock(nn.Module):
  ...
  @nn.compact
  def __call__(self, inputs, moment):
    # Attention Block.
...
class EncoderMoe(nn.Module):
  ...
  def __call__(self, inputs):
    ...
    metrics = {}
    moment = jnp.zeros_like(x)
    for block in range(self.num_layers):
      if block in moe_mlp_layers:
        x, moment, metrics[f'encoderblock_{block}'] = encoder_block_cls(
            name=f'encoderblock_{block}', mlp_block=moe_mlp_cls)(x, moment)
      else:
        x, moment = encoder_block_cls(
            name=f'encoderblock_{block}', mlp_block=dense_mlp_cls)(x, moment)
